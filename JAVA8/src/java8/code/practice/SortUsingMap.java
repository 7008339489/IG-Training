...
public class SortUsingMap {
	...
	public static void main(String[] args) {
		...
		//Sorting of Map<Employee, Integer>
		...
		Map<Employee, Integer> newMpEmp = mpEmp.entrySet().stream()
				.sorted(Map.Entry.comparingByKey(Comparator.comparing(Employee::getYearOfJoining)))
				.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (e1, e2) -> e1, LinkedHashMap::new));
		newMpEmp.forEach((k, v) -> System.out.println(k + " " + v));
		
		System.out.println("--------------------");
		
		mpEmp.entrySet().stream().sorted(Map.Entry.comparingByValue()).forEach(System.out::println);
	}
